...
export default function App() {
  ...
  const handleExportJson = () => {
    ...
    const exportFileDefaultName = `worklogs_${format(new Date(), 'yyyyMMdd_HHmm')}.json`;
  ...
  const handleImportJson = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        alert(`${count}개의 데이터가 성공적으로 동기화되었습니다.`);
      } catch (error) {
  ...
  const handleFetchJson = async () => {
    ...
    setIsSyncing(true);
    try {
      // Simulation of fetching from a remote JSON source
      const mockJsonData = [
        {
          date: format(new Date(), 'yyyy-MM-dd'),
          title: "시스템 자동 동기화",
          content: "클라우드와 JSON 데이터가 실시간으로 동기화되고 있습니다.",
          author: "시스템",
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 pr-4 border-r border-zinc-200">
              <button 
                ...
                className="btn btn-secondary p-2 text-zinc-500 hover:text-zinc-900"
                title="JSON 내보내기"
              >
                <FileJson size={18} />
              </button>
              <label className="cursor-pointer rounded-lg p-2 text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900 transition-colors" title="JSON 가져오기">
                <Plus size={18} />
                <input 
                  type="file" 
                  accept=".json" 
